...
use asconcore::{
    AsconCore, MaskedAsconCore, Parameters, Parameters128, Parameters128a, Parameters80pq,
};
...
// #[derive(Clone)]
// struct MaskedAscon<P: Parameters> {
//     key: P::InternalKey,
// }

// impl<P: Parameters> KeySizeUser for MaskedAscon<P> {
//     type KeySize = P::KeySize;
// }

// impl<P: Parameters> KeyInit for MaskedAscon<P> {
//     fn new(key: &Key<Self>) -> Self {
//         Self {
//             key: P::InternalKey::from(key),
//         }
...
// impl<P: Parameters> AeadCore for MaskedAscon<P> {
//     type NonceSize = U16;
//     type TagSize = U16;
//     type CiphertextOverhead = U0;
// }

// impl<P: Parameters> AeadInPlace for MaskedAscon<P> {
//     fn encrypt_in_place_detached(
//         &self,
//         nonce: &Nonce<Self>,
//         associated_data: &[u8],
...
//             return Err(Error);
//         }
//         // Really inefficient use of rng but big refactor is too much effort atm
//         // Also uses the same rng seed for every encryption / decryption, which is big no no
//         let rng = ChaCha20Rng::seed_from_u64(0xDEADBEEFCAFEF00D);
//         let mut core = MaskedAsconCore::<P, ChaCha20Rng>::new(&self.key, nonce, rng);
//         Ok(core.encrypt_inplace(buffer, associated_data))
//     }
...
// /// Ascon-128
// pub struct MaskedAscon128(MaskedAscon<Parameters128>);
// /// Key for Ascon-128
// pub type MaskedAscon128Key = Key<MaskedAscon128>;
// /// Nonce for Ascon-128
// pub type MaskedAscon128Nonce = Nonce<MaskedAscon128>;
// /// Tag for Ascon-128
// pub type MaskedAscon128Tag = Tag<MaskedAscon128>;

// impl KeySizeUser for MaskedAscon128 {
//     type KeySize = U16;
// }

// impl KeyInit for MaskedAscon128 {
//     fn new(key: &Key<Self>) -> Self {
//         Self(MaskedAscon::<Parameters128>::new(key))
//     }
// }

// impl AeadCore for MaskedAscon128 {
//     type NonceSize = U16;
//     type TagSize = U16;
//     type CiphertextOverhead = U0;
// }

// impl AeadInPlace for MaskedAscon128 {
//     #[inline(always)]
//     fn encrypt_in_place_detached(
//         &self,
//         nonce: &Nonce<Self>,
...
#[derive(Clone)]
struct MaskedAscon<P: Parameters, R: CryptoRngCore> {
    key: P::InternalKey,
    rng: R,
}

impl<P: Parameters, R: CryptoRngCore> KeySizeUser for MaskedAscon<P, R> {
    type KeySize = P::KeySize;
}
...
impl<P: Parameters, R: CryptoRngCore> MaskedAscon<P, R> {
    pub fn new(key: &Key<Self>, rng: R) -> Self {
        Self {
            key: P::InternalKey::from(key),
            rng,
        }
...
impl<P: Parameters, R: CryptoRngCore + Clone> AeadCore for MaskedAscon<P, R> {
    type NonceSize = U16;
    type TagSize = U16;
    type CiphertextOverhead = U0;
}

impl<P: Parameters, R: CryptoRngCore + Clone> AeadInPlace for MaskedAscon<P, R> {
    fn encrypt_in_place_detached(
        &self,
        nonce: &Nonce<Self>,
        associated_data: &[u8],
        ...
        // Also uses the same rng seed for every encryption / decryption, which is big no no
        // let rng = ChaCha20Rng::seed_from_u64(0xDEADBEEFCAFEF00D);
        let mut core = MaskedAsconCore::<P, R>::new(&self.key, nonce, self.rng.clone());
        Ok(core.encrypt_inplace(buffer, associated_data))
    }
...
/// Ascon-128
pub struct MaskedAscon128<R: CryptoRngCore + Clone>(MaskedAscon<Parameters128, R>);
/// Key for Ascon-128
pub type MaskedAscon128Key<R> = Key<MaskedAscon128<R>>;
/// Nonce for Ascon-128
pub type MaskedAscon128Nonce<R> = Nonce<MaskedAscon128<R>>;
/// Tag for Ascon-128
pub type MaskedAscon128Tag<R> = Tag<MaskedAscon128<R>>;

impl<R: CryptoRngCore + Clone> KeySizeUser for MaskedAscon128<R> {
    type KeySize = U16;
}
...
impl<R: CryptoRngCore + Clone> MaskedAscon128<R> {
    pub fn new(key: &Key<Self>, rng: R) -> Self {
        Self(MaskedAscon::<Parameters128, R>::new(key, rng))
    }
}

impl<R: CryptoRngCore + Clone> AeadCore for MaskedAscon128<R> {
    type NonceSize = U16;
    type TagSize = U16;
    type CiphertextOverhead = U0;
}

impl<R: CryptoRngCore + Clone> AeadInPlace for MaskedAscon128<R> {
    #[inline(always)]
    fn encrypt_in_place_detached(
        &self,
        nonce: &Nonce<Self>,
